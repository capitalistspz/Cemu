...
static void PPCInterpreter_STWCX(PPCInterpreter_t* hCPU, uint32 Opcode)
{
	...
	if (hCPU->reservedMemAddr == ea)
	{
		...
		{
			wordPtr = (uint32be*)(memory_base + ppcItpCtrl::ppcMem_translateVirtualDataToPhysicalAddr(hCPU, ea));
			DSI_EXIT();
		}
		else
		{
			wordPtr = (uint32be*)memory_getPointerFromVirtualOffset(ea);
		}
		auto wordRef = rawPtrToAtomicRef(wordPtr);
		uint32be newValue = hCPU->gpr[rS];
		if (!wordRef.compare_exchange_strong(reservedValue, newValue))
		{
			// failed
